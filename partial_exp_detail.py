...
def print_memory_usage(label=""):
    ...
    # RSS: 进程占用的物理内存
    # VMS: 进程占用的虚拟内存
    print(f"[{label}] 当前内存使用:")
    print(f"  - RSS (物理内存): {memory_info.rss / 1024 / 1024:.2f} MB")
    print(f"  - VMS (虚拟内存): {memory_info.vms / 1024 / 1024 / 1024:.2f} GB")
    ...
    if torch.cuda.is_available():
        for i in range(torch.cuda.device_count()):
            print(f"GPU {i} 内存: 已分配={torch.cuda.memory_allocated(i) / 1024 / 1024:.2f}MB, 缓存={torch.cuda.memory_reserved(i) / 1024 / 1024:.2f}MB")

def clean_memory(aggressive=False):
    """更彻底地清理内存"""
    # 正常的内存清理
    gc.collect()
    if torch.cuda.is_available():
    ...
    # 积极的内存清理（可选）
    if aggressive:
        # 尝试通过重新分配大量内存并立即释放来触发更彻底的垃圾回收
        try:
            print("执行积极内存清理...")
            temp = []
            # 尝试分配大数组并立即释放
            for _ in range(5):
                temp.append(torch.zeros(1024, 1024, 1024))
            del temp
            # 再次进行垃圾回收
            gc.collect()
            if torch.cuda.is_available():
...
class MemoryTracker:
    ...
    def record(self, label=""):
        ...
        record = {
            ...
            "system_used_gb": (virtual_memory.total - virtual_memory.available) / 1024 / 1024 / 1024,
    ...
    def save_to_file(self, filename="memory_usage.json"):
        """保存内存记录到文件"""
        with open(filename, "w") as f:
            json.dump(self.memory_records, f, indent=2)
        print(f"内存使用记录已保存到 {filename}")
    
    def plot(self, filename="memory_usage.png"):
        """绘制内存使用图表"""
        ...
        system_percent = [record["system_percent"] for record in self.memory_records]
        
        plt.figure(figsize=(12, 8))
        
        plt.subplot(2, 1, 1)
        plt.plot(times, rss_values, 'b-', label='进程RSS (MB)')
        plt.ylabel('内存使用 (MB)')
        plt.title('进程内存使用')
        plt.legend()
        plt.grid(True)
        ...
        plt.subplot(2, 1, 2)
        plt.plot(times, system_percent, 'r-', label='系统内存使用率 (%)')
        plt.xlabel('时间 (秒)')
        plt.ylabel('使用率 (%)')
        plt.title('系统内存使用率')
        plt.legend()
...
class ExpertUsageLogger:
    """专家使用情况记录器"""
    def __init__(self, output_dir="expert_usage_logs"):
        ...
        self.log_file = self.output_dir / f"expert_usage_{timestamp}.json"
    ...
    def log_token_experts(self, token_idx, token_text, experts_by_layer):
        """记录单个token的专家使用情况"""
        token_data = {
            "token_idx": token_idx,
        ...
        self.usage_data["tokens"].append(token_data)
    
    def log_overall_stats(self, stats):
        """记录总体统计信息"""
        self.usage_data["overall_stats"] = stats
    
    def save_to_file(self):
        """保存日志到文件"""
        with open(self.log_file, "w", encoding="utf-8") as f:
            json.dump(self.usage_data, f, ensure_ascii=False, indent=2)
        print(f"专家使用情况已保存到 {self.log_file}")

class SelectiveExpertsMoeInference:
    """按需加载专家的MOE模型推理类"""
    def __init__(self, model_path, experts_dir=None, device="cpu", track_memory=True, log_experts=True):
        ...
        if track_memory:
            ...
            self.memory_tracker.record("初始化开始")
        
        # 初始化专家使用记录器
        self.log_experts = log_experts
        if log_experts:
            self.expert_logger = ExpertUsageLogger()
        ...
        if self.track_memory:
            self.memory_tracker.record("加载分词器后")
        ...
        if self.track_memory:
            self.memory_tracker.record("加载配置后")
        ...
        if self.track_memory:
            self.memory_tracker.record("初始化旋转位置编码后")
        ...
        print("初始化完成")
        if self.track_memory:
            self.memory_tracker.record("初始化完成")
    ...
    def process_all_layers(self, prompt):
        ...
        if self.track_memory:
            self.memory_tracker.record("开始处理")
        ...
        if self.track_memory:
            self.memory_tracker.record("加载词嵌入后")
        ...
        # 如果记录专家使用情况，为每个token创建记录
        if self.log_experts:
            token_experts_usage = [{} for _ in range(input_length)]
        ...
        for layer_idx in range(self.config.num_hidden_layers):
            ...
            if self.track_memory:
                self.memory_tracker.record(f"开始处理层 {layer_idx}")
            ...
            if self.track_memory:
                self.memory_tracker.record(f"层 {layer_idx} 加载基本结构后")
            ...
            # 如果记录专家使用情况，记录每个token的专家分配
            if self.log_experts:
                # 获取每个token选择的专家
                # selected_experts的形状是 [batch_size*seq_len, num_experts_per_tok]
                # 将其重塑为 [batch_size, seq_len, num_experts_per_tok]
                token_selected_experts = selected_experts.view(1, input_length, self.config.num_experts_per_tok)
                
                for token_idx in range(input_length):
                    token_experts = token_selected_experts[0, token_idx].tolist()
                    token_experts_usage[token_idx][f"layer_{layer_idx}"] = token_experts
            ...
            if self.track_memory:
                self.memory_tracker.record(f"层 {layer_idx} 路由决策后")
            ...
            if self.track_memory:
                self.memory_tracker.record(f"层 {layer_idx} 加载专家权重后")
            ...
            if self.track_memory:
                self.memory_tracker.record(f"层 {layer_idx} 处理完成")
        ...
        if self.track_memory:
            self.memory_tracker.record("应用语言模型头后")
        ...
        if self.track_memory:
            self.memory_tracker.record("处理完成")
        ...
        if self.log_experts:
            # 创建详细的专家使用记录
            for token_idx, token_text in enumerate(tokens_text):
                self.expert_logger.log_token_experts(
                    token_idx=token_idx,
                    token_text=token_text,
                    experts_by_layer=token_experts_usage[token_idx]
                )
            
            # 记录总体统计信息
            overall_stats = {
                "input_text": output_text,
                "total_experts_needed": total_experts_needed,
                "total_experts_available": total_experts_available,
                "overall_memory_saving": overall_memory_saving,
                "per_layer_stats": experts_stats
            }
            self.expert_logger.log_overall_stats(overall_stats)
            self.expert_logger.save_to_file()
        
        # 如果开启了内存跟踪，保存记录
        if self.track_memory:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            self.memory_tracker.save_to_file(f"memory_usage_process_{timestamp}.json")
            self.memory_tracker.plot(f"memory_usage_process_{timestamp}.png")
    ...
    def generate_text(self, prompt, max_new_tokens=20):
        ...
        if self.track_memory:
            self.memory_tracker.record("生成开始")
        ...
        for i in range(max_new_tokens):
            ...
            if self.track_memory:
                self.memory_tracker.record(f"开始生成token {i+1}")
            ...
            for layer_idx in range(self.config.num_hidden_layers):
                ...
                # 如果记录专家使用情况，获取最后一个token（当前预测token）选择的专家
                if self.log_experts:
                    # 从batch*seq_len形状中获取最后一个token的专家选择
                    # 注意这里应该是获取序列中最后一个位置的token
                    last_token_idx = input_length - 1
                    last_token_offset = last_token_idx  # 在flatten后的索引
                    last_token_experts = selected_experts[last_token_offset].tolist()
                    current_token_experts[f"layer_{layer_idx}"] = last_token_experts
            ...
            # 如果记录专家使用情况，记录当前生成的token
            if self.log_experts:
                current_token_id = next_token_id[0].item()
                generated_tokens_info.append({
                    "token_idx": len(generated_tokens_info),
                    "token_id": current_token_id,
                    "token_text": current_token,
                    "is_input": False,
                    "experts_usage": current_token_experts,
            ...
            if self.track_memory:
                self.memory_tracker.record(f"生成token {i+1} 完成")
        ...
        if self.track_memory:
            self.memory_tracker.record("生成结束")
        
        # 如果记录专家使用情况，保存到文件
        if self.log_experts:
            # 创建新的记录文件，特别用于生成文本
            gen_expert_logger = ExpertUsageLogger("expert_usage_generation_logs")
            ...
            overall_stats = {
                ...
                "average_token_time": total_time/token_count if token_count > 0 else 0
            }
            gen_expert_logger.usage_data["overall_stats"] = overall_stats
            
            # 添加详细的token信息
            gen_expert_logger.usage_data["tokens"] = generated_tokens_info
            
            # 保存到文件
            gen_expert_logger.save_to_file()
        
        # 保存内存使用记录
        if self.track_memory:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            self.memory_tracker.save_to_file(f"memory_usage_generation_{timestamp}.json")
            self.memory_tracker.plot(f"memory_usage_generation_{timestamp}.png")
...
def main():
    ...
    # 创建一个全局内存跟踪器用于比较
    global_memory_tracker = MemoryTracker()
    global_memory_tracker.record("程序开始")
    ...
    try:
        ...
        # 清理内存
        clean_memory(aggressive=True)
        global_memory_tracker.record("清理内存后")
        
        print("\n===== 执行内存基线对比测试 =====")
        # 先进行一次基础推理来建立基线
        print("\n===== 1. 基础推理测试 - 使用选择性专家加载 =====")
        selective_inferencer = SelectiveExpertsMoeInference(
            model_path, 
        ...
        global_memory_tracker.record("初始化选择性专家推理器后")
        
        # 进行推理
        print("\n===== 方法1: 选择性专家加载方式处理所有层 =====")
        result = selective_inferencer.process_all_layers(prompt)
        global_memory_tracker.record("选择性专家推理完成")
        
        # 清理内存
        del selective_inferencer
        clean_memory(aggressive=True)
        global_memory_tracker.record("选择性专家推理后清理内存")
        
        # 打印结果
        print(f"\n选择性专家加载结果:")
        print(f"总体专家内存节省率: {result['overall_memory_saving']*100:.2f}%")
        ...
        global_memory_tracker.record("重新初始化选择性专家推理器后")
        
        generation_result = selective_inferencer.generate_text(prompt, max_new_tokens=50)  # 减少token数量以加快测试
        global_memory_tracker.record("选择性专家生成文本完成")
        
        # 清理内存
        del selective_inferencer
        clean_memory(aggressive=True)
        global_memory_tracker.record("选择性专家生成后清理内存")
        ...
        # 保存全局内存使用记录
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        global_memory_tracker.save_to_file(f"memory_usage_global_{timestamp}.json")
        global_memory_tracker.plot(f"memory_usage_global_{timestamp}.png")
