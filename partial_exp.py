...
from qwen3_moe.modeling_qwen3_moe import Qwen3MoeDecoderLayer, Qwen3MoeConfig, Qwen3MoeRMSNorm, Qwen3MoeRotaryEmbedding, Qwen3MoeSparseMoeBlock, Qwen3MoeMLP
...
class SelectiveExpertsMoeInference:
    ...
    def _create_empty_layer(self, layer_idx):
        ...
        with torch.no_grad():
            ...
            if hasattr(layer, 'mlp') and isinstance(layer.mlp, Qwen3MoeSparseMoeBlock):
                ...
                # 使用 torch.nn.Identity 作为无参数占位符
                layer.mlp.experts = torch.nn.ModuleList([torch.nn.Identity() for _ in range(num_experts)])

            # 强制进行垃圾回收
            clean_memory()
            print_memory_usage(f"创建并清理空层 {layer_idx} 后")
    ...
    def _load_expert_weights(self, layer, layer_idx, expert_idx):
        ...
        # 1. 创建一个新的专家模块，并确保使用正确的moe_intermediate_size
        with torch.no_grad():
            expert_config = copy.copy(self.config)
            expert_config.intermediate_size = self.config.moe_intermediate_size
            
            new_expert = Qwen3MoeMLP(expert_config).to(self.device)
            new_expert.eval()
            for param in new_expert.parameters():
                param.requires_grad_(False)

        # 2. 将权重加载到新专家中
        tensors = load_file(expert_path.as_posix(), device=self.device)
        
        # 定义需要从张量名称中移除的前缀
        prefix_to_strip = f"model.layers.{layer_idx}.mlp.experts.{expert_idx}."
        
        for name, tensor in tensors.items():
            # 检查张量名称是否具有预期的前缀
            if not name.startswith(prefix_to_strip):
                print(f"警告: 在专家 {expert_idx} 文件中跳过意外的张量: {name}")
                continue

            param_in_expert_name = name[len(prefix_to_strip):] # e.g., "gate_proj.weight"
            
            # 导航到正确的参数
            current_obj = new_expert
            parts = param_in_expert_name.split('.')
            for part in parts[:-1]:
                current_obj = getattr(current_obj, part)
            
            param = getattr(current_obj, parts[-1])

            # 复制权重
            with torch.no_grad():
                param.copy_(tensor.to(dtype=torch.bfloat16))
        
        # 3. 将加载好的专家模块放回层中
        with torch.no_grad():
            layer.mlp.experts[expert_idx] = new_expert
    ...
    def process_all_layers(self, prompt):
        ...
        for layer_idx in range(self.config.num_hidden_layers):
            ...
            # 测量加载完所有所需专家后的内存占用
            print_memory_usage(f"第 {layer_idx} 层加载 {len(unique_experts)}/{self.config.num_experts} 个专家后")
    ...
    def generate_text(self, prompt, max_new_tokens=5):
        """使用选择性专家加载方式生成文本"""
        ...
        for i in range(max_new_tokens):
            ...
            for layer_idx in range(self.config.num_hidden_layers):
                ...
                # 测量加载完所有所需专家后的内存占用
                print_memory_usage(f"生成token {i+1} - 第 {layer_idx} 层加载 {len(unique_experts)}/{self.config.num_experts} 个专家后")
...
def main():
    ...
    try:
        ...
        # 方法2: 生成多个token形成文本
        print("\n===== 方法2: 选择性专家加载方式生成文本 =====")
        generation_result = inferencer.generate_text(prompt, max_new_tokens=5) # 减少token数量以加快测试
